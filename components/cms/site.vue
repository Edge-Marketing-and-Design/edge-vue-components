...
const domainError = computed(() => {
  const siteLevelError = String(siteData.value?.domainError || '').trim()
  if (siteLevelError)
    return siteLevelError
...
const activeDomainError = computed(() => {
  ...
  if (persisted)
    return persisted
  return String(domainRegistryHeaderError.value || '').trim()
})
const normalizeHeaderDomain = (value) => {
  if (!value)
    return ''
  return String(value).trim().toLowerCase().replace(/\.+$/g, '')
}
const headerDomains = computed(() => {
...
<template>
  ...
  >
    ...
    <div v-else class="flex flex-col h-[calc(100vh-78px)] overflow-visible">
      <div
      ...
      >
        ...
        <div class="relative flex items-center gap-3 justify-end">
          <input
            ref="pageImportInputRef"
            type="file"
            multiple
